(() => {
  ...
  const OPTIONAL_STAGES = ['VERBAL_CONFIRMATION', 'WON', 'LOST', 'ON_HOLD'];
  ...
  const normalizeStage = (value) => {
    ...
    return ALL_STAGES.includes(normalized) ? normalized : 'NEW';
  };
  ...
  function createDealCard(item) {
    ...
    if (CLOSED_STAGES.has(item.stage)) article.dataset.akariClosedOutcome = item.stage;
    ...
    nextAction.textContent = item.next_action || (item.stage === 'WON' ? 'Client onboarding' : item.stage === 'LOST' ? 'Closed lost' : 'No next action');
    nextAction.style.color = (!item.next_action && !CLOSED_STAGES.has(item.stage)) || isOverdue(item.next_follow_up_at)
      ? 'var(--red)'
      : 'var(--muted-2)';
  ...
  function createStageColumn(stage, items) {
    ...
    section.dataset.akariOpportunityVisibility = 'r26';
    if (stage === 'NEW' || stage === 'RESEARCH') section.dataset.akariEarlyStage = stage;
    if (CLOSED_STAGES.has(stage)) section.dataset.akariClosedStage = stage;
    ...
    summary.textContent = `${items.length} ${items.length === 1 ? 'opportunity' : 'opportunities'} · ${money(value, 'USD', true)}`;
  ...
  async function mount() {
    ...
    try {
      ...
      const commercial = opportunities
        .map((item) => ({ ...item, stage: normalizeStage(item.stage) }))
        .filter((item) => !String(item.service_type || '').trim().toUpperCase().includes('FUNDRAISING'));

      const populatedOptionalStages = OPTIONAL_STAGES.filter((stage) => commercial.some((item) => item.stage === stage));
      const visibleStages = [...CORE_STAGES, ...populatedOptionalStages];
      const fragment = document.createDocumentFragment();
      for (const stage of visibleStages) {
        fragment.appendChild(createStageColumn(stage, commercial.filter((item) => item.stage === stage)));
      }
      pipeline.replaceChildren(fragment);
      pipeline.dataset.akariOpportunityVisibility = 'r26';
      pipeline.dataset.akariOpportunityCount = String(commercial.length);
      pipeline.dataset.akariOpenOpportunityCount = String(commercial.filter((item) => !CLOSED_STAGES.has(item.stage)).length);
      pipeline.dataset.akariClosedOpportunityCount = String(commercial.filter((item) => CLOSED_STAGES.has(item.stage)).length);
