(() => {
  ...
  const option=(value,current,display=null)=>`<option value="${esc(value)}" ${String(value)===String(current)?'selected':''}>${esc(display||label(value))}</option>`;
  function tone(status){return status==='APPROVED'?'approved':status==='READY_FOR_APPROVAL'?'ready':status==='REJECTED'?'rejected':status==='CHANGES_AFTER_APPROVAL'?'drift':'draft';}
  ...
  function modal(title,body,submitLabel,onSubmit){
    ...
    layer.addEventListener('click',(e)=>{if(e.target===layer||e.target.closest('[data-close]'))closeModal();});
    layer.querySelector('form').addEventListener('submit',async(e)=>{
      e.preventDefault(); const button=e.submitter; button.disabled=true;
      try{await onSubmit(new FormData(e.currentTarget));closeModal();}catch(error){alert(error.message);button.disabled=false;}
    });
    document.body.appendChild(layer);
  }
  function partnerOptions(partners,current){
    return [`<option value="">Direct / unassigned</option>`,...(partners||[]).map((p)=>`<option value="${esc(p.id)}" ${p.id===(current.agencyPartnerId||'')?'selected':''}>${esc(p.name)} · ${esc(label(p.partner_type||'Partner'))}</option>`)].join('');
  }
  function editItem(item){
    modal(`Edit plan item · ${item.name||item.handle||'Talent'}`,`
      <input type="hidden" name="id" value="${esc(item.id)}">
      <label>Delivery partner<select name="agencyPartnerId">${partnerOptions(payload?.deliveryPartners||[],item)}</select></label>
      ...
      <label class="full">Planning notes<textarea name="notes" rows="3">${esc(item.notes||'')}</textarea></label>`,
      'Save plan item',async(form)=>patch({action:'upsert-plan-item',assignment:Object.fromEntries(form.entries())}));
  }
  function rejectPlan(){
  ...
  async function patch(body){
    if(!activeId)return;
    payload=await api(`/api/campaign-planning/${encodeURIComponent(activeId)}`,{method:'PATCH',body:JSON.stringify(body)});
    loadedId=activeId;
    render();
  }
  ...
  function actions(item,permissions){
    const planning=item.planning||{}; const effective=item.summary?.effectiveStatus||planning.status; const buttons=[];
  ...
  function render(){
    ...
    const item=payload.item, planning=item.planning||{}, summary=item.summary||{}, permissions=payload.permissions||{};
    const effective=summary.effectiveStatus||planning.status||'DRAFT';
    const editable=permissions.canWrite&&['DRAFT','REJECTED'].includes(planning.status);
    const selectedCampaign=campaigns.find((c)=>c.id===activeId);
    panel.innerHTML=`
      <header class="campaign-plan-head-r56"><div><span>CAMPAIGN PLANNING · R8.5F</span><strong>Talent Basket & Approval Workspace</strong><small>Turn recommendation intelligence into a governed execution plan without duplicating campaign data.</small></div><div class="campaign-plan-status-r56 ${tone(effective)}"><b>${esc(label(effective))}</b><span>${summary.approvalDrift?'Approved basket changed — reapproval required':'Current plan state'}</span></div></header>
      <div class="campaign-plan-selector-r56"><label>Campaign<select data-r56-campaign>${campaigns.map((c)=>option(c.id,activeId,`${c.name}${c.project_name?` · ${c.project_name}`:''}`)).join('')}</select></label><div><strong>${esc(selectedCampaign?.name||item.name)}</strong><span>${esc(item.projectName||'')} · ${esc(label(item.status||''))}</span></div>${actions(item,permissions)}</div>
    ...
    panel.querySelector('[data-r56-campaign]')?.addEventListener('change',(e)=>{activeId=e.target.value;loadedId='';loadPlan(true);});
    panel.querySelector('[data-save-plan]')?.addEventListener('click',async()=>{const next={};panel.querySelectorAll('[data-plan-field]').forEach((f)=>{next[f.dataset.planField]=f.dataset.planField==='budgetUsd'?Number(f.value||0):f.value;});try{await patch({action:'update-plan',planning:next});}catch(error){alert(error.message);}});
    ...
    panel.querySelectorAll('[data-plan-item]').forEach((row)=>{
      const item=(payload.item.planItems||[]).find((x)=>x.id===row.dataset.planItem);
      row.querySelector('[data-edit]')?.addEventListener('click',()=>editItem(item));
      row.querySelector('[data-remove]')?.addEventListener('click',async()=>{if(!confirm(`Remove ${item.name||item.handle||'this talent'} from the plan?`))return;try{await patch({action:'remove-plan-item',assignmentId:item.id});}catch(error){alert(error.message);}});
    });
  ...
  async function loadCampaigns(force=false){
    ...
    const response=await api('/api/campaigns'); campaigns=response.items||[]; loadedCampaigns=true;
    if(!activeId&&campaigns.length)activeId=(campaigns.find((c)=>!['COMPLETED','CANCELLED'].includes(String(c.status||'').toUpperCase()))||campaigns[0]).id;
  }
  async function loadPlan(force=false){
    const panel=shell(); if(!panel||loading)return;
    if(!force&&payload?.item&&loadedId===activeId)return;
    loading=true; if(force)panel.classList.add('is-loading');
    try{
      await loadCampaigns();
      if(!activeId){payload=null;loadedId='';render();return;}
  ...
  let queued=false;
  const queue=()=>{if(queued)return;queued=true;requestAnimationFrame(()=>{queued=false;if(!campaignPage()){payload=null;loadedCampaigns=false;loadedId='';activeId='';return;}loadPlan();});};
  new MutationObserver(queue).observe(document.body,{childList:true,subtree:true});
  window.addEventListener('popstate',queue);
  document.addEventListener('click',(e)=>{if(e.target.closest('[data-route],[data-action="new-campaign"]'))setTimeout(queue,0);},true);
  queue();
})();
