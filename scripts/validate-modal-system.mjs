...
for (const token of ['/assets/modal-system-r9.css?v=1', '/assets/select-option-contrast-v1.css?v=1', '/assets/modal-system-r9.js?v=1']) {
  if (!html.includes(token)) throw new Error(`Protected shell missing ${token}`);
}
for (const token of ['akari-crm-shell-v40', './assets/modal-system-r9.css?v=1', './assets/select-option-contrast-v1.css?v=1', './assets/modal-system-r9.js?v=1', 'runtime=v40']) {
  if (!sw.includes(token)) throw new Error(`Service worker missing ${token}`);
}

console.log('AKARI modal system, readable selects and Tasks navigation validation passed');
