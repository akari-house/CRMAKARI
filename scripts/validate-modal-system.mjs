...
for (const token of ['.ak-modal-standard', '.ak-modal--wide', '.commercial-form-grid', '.revenue-field-grid', 'scrollbar-gutter:stable', 'body.ak-modal-open', '.ak-combobox__trigger', '.ak-combobox__panel', '.ak-combobox__option', '.ak-combobox__native']) {
  if (!css.includes(token)) throw new Error(`Modal system CSS missing ${token}`);
}
for (const token of ['normalizeTaskNavigation', 'normalizeDialog', "controls.length >= 8", "[data-route=\"day\"]", 'Open Tasks', 'enhanceSelect', 'openCombobox', 'renderComboboxOptions', "role', 'combobox", 'data-ak-option-index']) {
