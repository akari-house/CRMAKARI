...
const coreApi = await readFile('functions/api/work-os-core.js', 'utf8');
...
for (const token of ['partial: true', 'FROM tasks t', 'FROM tenant_memberships', 'server-timing', "scope === 'mine'"]) if (!coreApi.includes(token)) throw new Error(`Fast Work OS core API missing ${token}`);
...
for (const token of ['ensureCanonicalMyDay', 'task-board-panel', 'data-work-drag-handle', 'persistMove', "action: 'update-task'", 'pointerdown', 'ArrowRight', '/api/work-os-core', 'progressiveWorkFetch', 'scheduleFullHydration', 'full=1', 'Tasks are taking longer than expected']) if (!canonicalJs.includes(token)) throw new Error(`Canonical My Day runtime missing ${token}`);
