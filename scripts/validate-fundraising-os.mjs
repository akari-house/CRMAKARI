...
for(const pattern of ['WHERE tenant_id = ?','INVESTOR_PIPELINE_CREATED','Owner, Admin or BD Manager','upsert-document','upsert-access','upsert-diligence','upsert-question','NDA must be signed'])if(!api.includes(pattern))throw new Error(`Fundraising validation missing: ${pattern}`);
