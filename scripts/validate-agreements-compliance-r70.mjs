...
expect(deploy.includes('apply_migration()'),'production deployment must use the controlled migration helper');
expect(deploy.includes("payload=\"$(jq -Rs '{sql: .}' \"${file}\")\""),'migration helper must create a JSON-safe SQL payload');
