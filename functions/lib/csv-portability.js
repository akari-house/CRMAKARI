...
export async function previewCsvImport(db,tenantId,entity,csv){
  ...
  for(let index=0;index<parsed.records.length;index++){
    ...
    if(key==='projects'){
      ...
      accepted.push({row,record:{name,website:website||null,category:text(record.category,100)||null,ecosystem:text(record.ecosystem,100)||null,country:text(record.country,100)||null,region:text(record.region,100)||null,lifecycleStatus:lifecycle,relationshipHealth:text(record.relationship_health,40)||null,notes:text(record.notes,4000)||null}});continue;
    }
...
export async function commitCsvImport(db,tenantId,userId,entity,csv){
  ...
  for(const item of preview.accepted){const r=item.record;if(preview.entity==='projects')await run(db,`INSERT INTO projects (id,tenant_id,name,website,category,ecosystem,country,region,lifecycle_status,relationship_health,notes,created_at,updated_at,created_by,updated_by) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,[makeId('project'),tenantId,r.name,r.website,r.category,r.ecosystem,r.country,r.region,r.lifecycleStatus,r.relationshipHealth,r.notes,stamp,stamp,userId,userId]);else await run(db,`INSERT INTO contacts (id,tenant_id,project_id,full_name,email,telegram,job_title,contact_role,x_handle,linkedin_url,phone,country,preferred_channel,relationship_strength,is_decision_maker,is_primary_contact,created_at,updated_at,created_by,updated_by) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,[makeId('contact'),tenantId,r.projectId,r.fullName,r.email,r.telegram,r.jobTitle,r.contactRole,r.xHandle,r.linkedinUrl,r.phone,r.country,r.preferredChannel,r.relationshipStrength,r.isDecisionMaker,r.isPrimaryContact,stamp,stamp,userId,userId]);}
