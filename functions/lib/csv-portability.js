...
const PROJECT_LIFECYCLES=new Set(['LEAD','PROSPECT','ACTIVE_OPPORTUNITY','CLIENT','DORMANT_CLIENT','FORMER_CLIENT','PARTNER','ARCHIVED']);
...
const EXPORTS={
  projects:{sql:`SELECT id,name,slug,website,category,ecosystem,country,region,lifecycle_status,relationship_health,priority,owner_user_id,last_activity_at,next_follow_up_at,created_at,updated_at FROM projects WHERE tenant_id=? ORDER BY updated_at DESC`,columns:['id','name','slug','website','category','ecosystem','country','region','lifecycle_status','relationship_health','priority','owner_user_id','last_activity_at','next_follow_up_at','created_at','updated_at']},
  contacts:{sql:`SELECT c.id,c.project_id,p.name project_name,c.full_name,c.email,c.telegram,c.job_title,c.contact_role,c.x_handle,c.linkedin_url,c.phone,c.country,c.preferred_channel,c.relationship_strength,c.is_decision_maker,c.is_primary_contact,c.last_contacted_at,c.next_follow_up_at,c.created_at FROM contacts c JOIN projects p ON p.id=c.project_id AND p.tenant_id=c.tenant_id WHERE c.tenant_id=? ORDER BY c.created_at DESC`,columns:['id','project_id','project_name','full_name','email','telegram','job_title','contact_role','x_handle','linkedin_url','phone','country','preferred_channel','relationship_strength','is_decision_maker','is_primary_contact','last_contacted_at','next_follow_up_at','created_at']},
...
export async function previewCsvImport(db,tenantId,entity,csv){
  ...
  for(let index=0;index<parsed.records.length;index++){
    ...
    if(key==='projects'){
      ...
      const website=text(record.website,500),lifecycle=(text(record.lifecycle_status,40).toUpperCase()||'PROSPECT');if(!PROJECT_LIFECYCLES.has(lifecycle)){errors.push({row,error:'lifecycle_status is invalid'});continue;}
      const duplicate=website?await first(db,`SELECT id FROM projects WHERE tenant_id=? AND lower(website)=lower(?)`,[tenantId,website]):await first(db,`SELECT id FROM projects WHERE tenant_id=? AND lower(name)=lower(?)`,[tenantId,name]);
      if(duplicate){errors.push({row,error:'project already exists'});continue;}
      accepted.push({row,record:{name,website:website||null,category:text(record.category,100)||null,ecosystem:text(record.ecosystem,100)||null,country:text(record.country,100)||null,region:text(record.region,100)||null,lifecycleStatus:lifecycle,relationshipHealth:text(record.relationship_health,40)||null,description:text(record.description||record.notes,4000)||null}});continue;
    }
    ...
    accepted.push({row,record:{projectId:project.id,fullName,email:contactEmail,telegram:text(record.telegram,200)||null,jobTitle:text(record.job_title,200)||null,contactRole:text(record.contact_role,100)||null,xHandle:text(record.x_handle,200)||null,linkedinUrl:text(record.linkedin_url,500)||null,phone:text(record.phone,100)||null,country:text(record.country,100)||null,preferredChannel:text(record.preferred_channel,100)||null,relationshipStrength:text(record.relationship_strength,100)||null,isDecisionMaker:truthy(record.is_decision_maker),isPrimaryContact:truthy(record.is_primary_contact)}});
  }
...
export async function commitCsvImport(db,tenantId,userId,entity,csv){
  ...
  for(const item of preview.accepted){const r=item.record;if(preview.entity==='projects')await run(db,`INSERT INTO projects (id,tenant_id,name,website,category,ecosystem,country,region,lifecycle_status,relationship_health,description,created_at,updated_at,created_by,updated_by) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,[makeId('project'),tenantId,r.name,r.website,r.category,r.ecosystem,r.country,r.region,r.lifecycleStatus,r.relationshipHealth,r.description,stamp,stamp,userId,userId]);else await run(db,`INSERT INTO contacts (id,tenant_id,project_id,full_name,email,telegram,job_title,contact_role,x_handle,linkedin_url,phone,country,preferred_channel,relationship_strength,is_decision_maker,is_primary_contact,created_at,updated_at,created_by,updated_by) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,[makeId('contact'),tenantId,r.projectId,r.fullName,r.email,r.telegram,r.jobTitle,r.contactRole,r.xHandle,r.linkedinUrl,r.phone,r.country,r.preferredChannel,r.relationshipStrength,r.isDecisionMaker,r.isPrimaryContact,stamp,stamp,userId,userId]);}
