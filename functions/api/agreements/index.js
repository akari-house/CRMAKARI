...
async function tableExists(db,name){return Boolean(await first(db,"SELECT name FROM sqlite_master WHERE type='table' AND name=? LIMIT 1",[name]));}
async function ensureSchema(db){if(!await tableExists(db,'agreements'))throw issue('Agreement Registry migration 0003 must be applied before this module is available',503);}
async function validateLinks(db,tenantId,body,input){
  ...
  const hasNormalizedFundraising=await tableExists(db,'fundraising_rounds');
  if((roundId||input.agreementType==='FUNDRAISING_MANDATE')&&!hasNormalizedFundraising)throw issue('Normalized fundraising migration 0002 is required before linking a fundraising mandate to a round',503);
...
async function listPayload(db,tenantId,auth){
  ...
  const [projects,opportunities,campaigns,partners,members]=await Promise.all([
  ...
  const rounds=hasNormalizedFundraising?await all(db,"SELECT id,project_id,round_name,stage,instrument FROM fundraising_rounds WHERE tenant_id=? ORDER BY updated_at DESC",[tenantId]):[];
  ...
  return{items,summary,options:{projects,opportunities,campaigns,partners,rounds,members},capabilities:{normalizedFundraising:hasNormalizedFundraising},permissions:{canWrite:WRITE_ROLES.has(auth.role),canFinalise:MANAGER_ROLES.has(auth.role)},methodology:{version:'R70-1',governanceTrackingOnly:true,legalDetermination:false,tenantScoped:true,audited:true}};
}

export async function onRequestGet(context){try{const auth=context.data.auth,tenantId=requireTenant(auth);if(!context.env.DB)return json({items:[],summary:{},options:{projects:[],opportunities:[],campaigns:[],partners:[],rounds:[],members:[]},capabilities:{normalizedFundraising:false},demo:true});return json(await listPayload(context.env.DB,tenantId,auth));}catch(cause){return error(cause.message||'Agreements could not be loaded',Number(cause.status||500));}}
