...
async function getDashboard(context) {
  ...
  const target = await first(context.env.DB, `SELECT gross_revenue_target, net_revenue_target FROM monthly_targets WHERE tenant_id = ? AND user_id IS NULL AND year = ? AND month = ? LIMIT 1`, [tenantId, monthStart.getUTCFullYear(), monthStart.getUTCMonth()+1]);
  const campaignTotals = await first(context.env.DB, `
    ...
    FROM campaigns WHERE tenant_id = ? AND created_at >= ? AND created_at < ?
  `, [tenantId, monthStart.toISOString(), nextMonth.toISOString()]);
  const collected = await first(context.env.DB, `SELECT COALESCE(SUM(amount),0) AS value FROM payments WHERE tenant_id = ? AND status = 'PAID' AND received_date >= ? AND received_date < ?`, [tenantId, monthStart.toISOString().slice(0,10), nextMonth.toISOString().slice(0,10)]);
  const pipeline = await first(context.env.DB, `SELECT COALESCE(SUM(weighted_value),0) AS value, COUNT(*) AS count FROM opportunities WHERE tenant_id = ? AND stage NOT IN ('WON','LOST')`, [tenantId]);
  const ytd = await first(context.env.DB, `SELECT COALESCE(SUM(amount),0) AS value FROM payments WHERE tenant_id = ? AND status = 'PAID' AND received_date >= ?`, [tenantId, yearStart.toISOString().slice(0,10)]);
  const counts = await first(context.env.DB, `
    SELECT
      COUNT(CASE WHEN lifecycle_status = 'CLIENT' THEN 1 END) AS active_customers,
      COUNT(CASE WHEN lifecycle_status = 'PARTNER' THEN 1 END) AS active_partners
    FROM projects WHERE tenant_id = ? AND lifecycle_status != 'ARCHIVED'
  `, [tenantId]);
  const outstanding = await first(context.env.DB, `SELECT COALESCE(SUM(amount),0) AS value FROM payments WHERE tenant_id = ? AND status IN ('INVOICED','PARTIALLY_PAID','OVERDUE')`, [tenantId]);
  const referrals = await first(context.env.DB, `SELECT COALESCE(SUM(referral_amount),0) AS value FROM referrals WHERE tenant_id = ? AND payment_status IN ('CONFIRMED','DUE')`, [tenantId]);
  ...
  return json(withFinanceGuard({
    ...
    metrics: {
      ...
      activeCustomers: Number(counts?.active_customers || 0),
      activeCampaigns: Number(campaignTotals?.active_campaigns || 0),
      activePartners: Number(counts?.active_partners || 0),
...
async function getProjects(context) {
  ...
  const items = await all(context.env.DB, `
    SELECT p.id, p.name, p.category, p.lifecycle_status, p.priority, p.last_activity_at, p.next_follow_up_at,
           ...
           COALESCE(SUM(CASE WHEN o.stage NOT IN ('WON','LOST') THEN o.estimated_value ELSE 0 END),0) AS pipeline_value,
           p.source_name
    FROM projects p
...
async function getProject(context, id) {
  if (inDemo(context)) {
    const item = DEMO_PROJECTS.find((p) => p.id === id || p.name.toLowerCase().replaceAll(' ','-') === id);
