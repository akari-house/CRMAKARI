...
export async function onRequestGet(context) {
  try {
    ...
    if (priority) { conditions.push('p.priority = ?'); bindings.push(priority.toUpperCase()); }
    ...
    const items = await all(context.env.DB, `
      SELECT p.id,p.name,p.category,p.lifecycle_status,p.priority,p.website,p.x_url,p.telegram,p.region,p.source_name,
        p.original_status,p.original_notes,p.last_activity_at,p.next_follow_up_at,p.created_at,u.full_name AS owner,
      ...
      ORDER BY CASE p.priority WHEN 'URGENT' THEN 1 WHEN 'HIGH' THEN 2 WHEN 'MEDIUM' THEN 3 ELSE 4 END,
        COALESCE(p.next_follow_up_at,'9999-12-31') ASC,p.name COLLATE NOCASE ASC
      LIMIT ? OFFSET ?
    `, [...bindings, limit, offset]);
    const enriched = items.map((item) => ({
      ...item,
      identity_complete: Boolean(item.x_url && item.telegram),
      contact_identity_complete: Number(item.contact_count || 0) === 0 ? false : Boolean(item.primary_contact_x && item.primary_contact_telegram),
    ...
    const categories = await all(context.env.DB, `SELECT COALESCE(category,'Uncategorized') AS category,COUNT(*) AS count FROM projects WHERE tenant_id=? AND source_type='AKARI_LEADS' GROUP BY COALESCE(category,'Uncategorized') ORDER BY count DESC,category ASC`, [tenantId]);
    return json({ items: enriched, total: Number(count?.total || 0), categories, limit, offset, canWrite: canWrite(auth), tenant: { id: tenantId, slug: auth.tenantSlug, name: auth.tenantSlug === 'akari-house' ? 'AKARI House' : auth.tenantSlug } });
  } catch (cause) {
