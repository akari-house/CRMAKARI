...
export async function onRequestGet(context) {
  try {
    ...
    return json({
      ...
      receipts: receipts.map((item) => ({ id:item.id, amount:Number(item.amount || 0), receivedDate:item.received_date, paymentMethod:item.payment_method, reference:item.wallet_or_bank_reference, createdAt:item.created_at })),
      credits: credits.map((item) => { const meta = parseJson(item.notes, {}); return { id:item.id, reference:item.invoice_reference, amount:Number(item.amount || 0), reason:meta.reason || null, issuedAt:meta.issuedAt || item.created_at, transactionReference:item.wallet_or_bank_reference || null }; }),
    });
  } catch (cause) {
...
export async function onRequestPatch(context) {
  try {
    ...
    if (action === 'issue') {
      ...
      const available = roundMoney(invoice.total - invoice.credited);
      if (amount > available + 0.005) return error('Credit-note amount exceeds the invoice total available for credit', 422);
      ...
      const status = credited + 0.005 >= invoice.total ? 'CREDITED' : 'PARTIALLY_CREDITED';
    ...
    await syncEngagement(context.env.DB, tenantId, row.campaign_id, auth, now);
    await run(context.env.DB, `
